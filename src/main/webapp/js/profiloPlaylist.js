...
function eliminaPlaylist(){
	var titolo = event.currentTarget.value;
	var elementoDaRimuovere = event.currentTarget.parentElement;
	$.ajax({
		url: "deletePlaylist",
		method: "POST",
		data: {titoloPlaylist: titolo},
...
function eliminaVideo(){
	...
	var classTit = tit.replace(" ","_");
	var a = document.getElementsByClassName(classTit);
	var elementoDaRimuovere;
	
	if(a.length === 1){
		elementoDaRimuovere = event.currentTarget.parentElement.parentElement.parentElement.parentElement;
	}	
	else{
		elementoDaRimuovere = event.currentTarget.parentElement.parentElement;
	}
	
	$.ajax({
		url: "deleteVideoPlaylist",
		method: "POST",
		data: {linkVideo: link, titoloPlaylist: tit},
		success: function(result){
			elementoDaRimuovere.remove();
		},
		error: function(error) {
